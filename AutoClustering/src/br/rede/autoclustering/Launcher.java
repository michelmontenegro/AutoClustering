...
public class Launcher {
	...
	public static void runVAC() {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
            	mainVAC=  new MainVAC();
            	mainVAC.setVisible(true);
            }
        });  
	}
	
    public static Launcher runAutoCluster(MainVAC mainAVC) {
        int runs = mainAVC.rodadas;//30
        int generations = mainAVC.geracoes;//500
        int population = mainAVC.individuos;//50
        int slices = 10;
        int sigma = 2;
        float epsilon = 0.3f;
        ...
        //File input = new File("uci-databases/heart-disease/processed.cleveland.data"); //ok pequeno segundo yuri , opção 2
//        File input = new File("uci-databases/glass/glass2.arff"); //ok pequeno segundo yuri
        ...
        //File input = new File("databases/agua_cat4_sem_CAT.arff");
        //File input = new File("databases/agua_cat_inspvis.arff");
        //File input = new File("databases/agua_com_cat__full_dry_6a.arff");
        //File input = new File("databases/ufpaFault.arff");
        //File input = new File("databases/nsl_kdd.arff");
        
        //File input = new File("uci-databases/heart-disease/processed.cleveland.arff"); //nao usar
        System.out.println( mainAVC.rodadas);
        return new Launcher(input, classifier, runs, generations, population, slices, sigma, epsilon, false);    	
    }
    ...
    public Launcher(File file3, Classifier classifier, int runs, int generations, int population, int slices, int dbSigma, float dbEpsilon, boolean verbose) {
        ...
        try {
            ...
            //for (int j = 12; j < runs; j++) {// usar quando interrompido
...
//				}
                ...
                diff = diff - runStarted;
                builder.append("Tempo de processamento da rodada: "+String.format("%6d", (diff / 60000))+"(min) e "+String.format("%2d", (diff / 1000)%60)+" (s)\r\n");
    ...
    private double runEDA(Collection<Node> nodes, Instances train, Classifier classifier, int generations, int population,
        ...
        try {
            best = Eda.getInstance().execute(nodes, train, classifier, generations, population,slices,writer,rodada,name, mainVAC);// adicionado a rodada atual
        } catch (Exception e1) {
            e1.printStackTrace();
        }
