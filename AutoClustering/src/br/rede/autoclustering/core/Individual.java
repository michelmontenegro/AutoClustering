...
public class Individual implements Comparable<Individual>, Runnable{
	...
	public int block;
	public SortedList<String> outBlock = new SortedList<String>();
	public SortedList<String> outBlockFit = new SortedList<String>();
	
	@Override
	public String toString() {
		return "Run: "+ Integer.toString(run)  +"| Gen: " + Integer.toString(generation) + "| Ind:" + Integer.toString(individuo) + "| Fit:" + Double.toString(fitness);
	}
	
	public Individual(ProbDag probDag) {
		this.probDag = probDag;
		run = Util.run;
		generation = Util.generation;
	}
	...
	public void execute(){
		Random r = new Random();
//		r.setSeed(1);
		//Cria a lista com os "individualNodel" que vão compor esta classe.
		
		//Adiciona o 1º node!
		nodes.add(new IndividualNode(probDag.getInitialNode()));
		
		//A partir do node inicial, sai varrendo os outros
		//Percorre todos os caminhos
		for (int i=0; nodes.get(i).getNode().getEdgesOut().size()>0; i++) {
			float probEdge = r.nextFloat();
			float totProb = 0f;
			
			/*Para cada um dos caminhos percorridos no for, ocorre um itarator do mesmo, percorrendo novamente estes caminhos
			 *porem o foco é usar as probabilidades das arestas (blocos) para decidir quais vão ser adicionadas/formadas aqui, atraves 
			 *do 'selEdge' */
			//getEdgesOut aponta para as saidas, a partir de uma origem, e ele anda nos vertices e vai pegando os "outs" dele.
			...
			//Cria efetivamente o bloco (Caracteristica) a ser trabalhado
			...
			//Adiciona o proximo node
			nodes.add(newNode);
			outBlock.add(Integer.toString(run)+";"+Integer.toString(generation)+";"+Integer.toString(individuo)+";"+Integer.toString(block)+";"+selEdge.toString()+";"+newNode.getProperties()+";"+selEdge.getProbability());
			block+=1;
		}
		this.setFitness(this.fitness(false));
		outBlockFit.clear();
		for (String line: outBlock) {
			outBlockFit.add(line+";"+getFitness());
		}
		outBlock.clear();
	}
