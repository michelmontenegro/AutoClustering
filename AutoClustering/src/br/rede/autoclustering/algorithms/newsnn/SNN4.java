...
public class SNN4 {
	...
	public static void main(String[] args) throws Exception {
		...
		//db.loadDataBaseCSV(new File("databases/sinteticDataXY.in"));// 4 clusters
...
//		Instances instances = db.getDatabase();
		...
		List<Group> groups = snn(instances,20, 10, 5);// snn(dataset, k, Eps, MinPts)
	...
	public static List<Group> snn(Instances X, int K, double Eps, int MinPts) throws Exception {
		...
		//O hashMap fará com que uma instância esteja associada a um inteiro
		...
		//O primeiro elemento no kns é a própria instância e os demais são os k-vizinhos.
		//Acaba que o número da linha é a instância em análise e o kns são os k-vizinhos.
		...
		for (int i = 0; i < N; i++) {
			...
			//System.out.println("Num of instances for nns: "+nns.numInstances());
...
//				System.out.println("#####"+nns.instance(j));
//				System.out.println("*****"+(Integer) getIndex(X, nns.instance(j)));
//				System.out.println("!!!!!"+X.instance((Integer) getIndex(X, nns.instance(j))));
				hs.add((Integer) getIndex(X, nns.instance(j)));
			}

			kns.put(i, hs);
			//System.out.println(kns.get(i).toString());
		}
		
		for (int i = 0; i < kns.size(); i++) {
			System.out.println("KNS"+i+" com k-vizinhos: "+kns.get(i).size());
		}
		
//		System.out.println("Tamanho do kns: "+kns.size());
//		for (int i = 0; i < 20; i++) {
//			System.out.print("Linha: "+i+" do kns: ");
...
//		Instance temp = X.instance(0);
//		System.out.println(temp);
////		System.out.println("asdfasf: "+kns.get(temp));
//		for (Integer a : kns.get(0)) {
//			System.out.print("Chave: "+a+" assocciada a instância: "+X.instance(a)+",");
//		}
...
//					System.out.println("");
		...
		System.out.println("Tamanho de 'S': ");
		System.out.println("Rows: "+S.rows()+" Columns: "+S.columns());
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 20; j++) {
...
//			System.out.println("");
		...
		for (int i = 0; i < N-1; i++) {
			int j = i+1;
			Integer instX = getIndex(X, X.instance(i));
			Integer instY = getIndex(X, X.instance(j));
			ArrayList<Integer> inst1 = kns.get(instX);
			ArrayList<Integer> inst2 = kns.get(instY);
			
			//ArrayList<Integer> sharedObjects = getSharedObjects(inst1, inst2);
			if (kns.get(instX).contains(instY) && kns.get(instY).contains(instX)) {
				continue;
			}else
			{
				cf++;
		...
		System.out.println("cf Output: "+cf);
		

//		ArrayList<Integer> sharedObjects = getSharedObjects(kns.get(X1), kns.get(X2));
//		System.out.println(sharedObjects.toString());
...
//		System.out.println(S.toCSV());
		...
		/*//debug
		 * int f = 0;
		System.out.println("Output Dens: ");
		for (int i =0; i< snnDens.length;i++) {
			f++;
			System.out.print(snnDens[i]+" | ");
			if (f == 15) {
				System.out.println("");
				f = 0;
			}
